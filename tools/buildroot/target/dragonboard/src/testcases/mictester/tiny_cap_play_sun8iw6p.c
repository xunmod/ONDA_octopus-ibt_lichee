...
void set_to_mic1()
{
	...
	tinymix_set_value_byname(mixer,"AIF1 AD0L Mixer ADCL Switch",1);
	tinymix_set_value_byname(mixer,"AIF1 AD0R Mixer ADCR Switch",1);
	tinymix_set_value_byname(mixer,"ADCR Mux", 0);
	tinymix_set_value_byname(mixer,"ADCL Mux", 0);
	tinymix_set_value_byname(mixer,"ADC volume", 0xc0);

	tinymix_set_value_byname(mixer,"LEFT ADC input Mixer MIC1 boost Switch", 1);
	tinymix_set_value_byname(mixer,"RIGHT ADC input Mixer MIC1 boost Switch", 1);
...
static void *music_play(void *args)
{
    ...
    play_config.channels = 2;
    play_config.rate = samplerate;
    ...
    play_pcm = pcm_open(1, 0, PCM_OUT, &play_config);//open audio codec
    if (!play_pcm || !pcm_is_ready(play_pcm)) {
        fprintf(stderr, "Unable to open PCM device (%s)\n",
                pcm_get_error(play_pcm));
        pthread_exit((void *)-1);
    }
    ...
    size = pcm_frames_to_bytes(play_pcm, pcm_get_buffer_size(play_pcm));
    buffer = malloc(size);
    if (!buffer) {
    ...
    db_msg("mictester Playing sample: %u ch, %u hz, 16 bit\n", play_config.channels, play_config.rate);

    do {
        num_read = fread(buffer, 1, size, fp);
        if (num_read > 0) {
            if (pcm_write(play_pcm, buffer, num_read)) {
                            ...
                            unsigned int bits,int argc, char **argv)
{
    ...
    cap_pcm = pcm_open(1, device, PCM_IN, &cap_config);

    #if 0
    if(disp_output_type_t==DISP_OUTPUT_TYPE_HDMI){
        play_pcm = pcm_open(0, device, PCM_OUT, &play_config);//open hdmi audio codec
    }else
    {
        play_pcm = pcm_open(1, device, PCM_OUT, &play_config);//open audio codec
    }
    #endif
