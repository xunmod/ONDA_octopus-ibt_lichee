...
int inv_get_silicon_rev_mpu6050(struct inv_mpu_iio_s *st)
{
	...
	result = inv_i2c_read(st, REG_XA_OFFS_L_TC, SOFT_PROD_VER_BYTES, regs);
	if (result)
		return result;
	...
	if (sw_rev == 0) {
		...
		/*return -EINVAL;*/
		printk
		    ("%s, silicon id not match, use latest id map:sw_rev_map[2]\n",
		     __func__);
		sw_rev = 2;
		p_rev = (struct prod_rev_map_t *)&sw_rev_map[sw_rev];
	}
					...
					int *reg_avg, int accel_thres_mg)
{
	...
	accel_bias[Z] =
	    abs(abs(reg_avg[Z] * st->chip_info.multi) -
		DEF_ST_PRECISION * DEF_ST_SCALE /
		(ACCL_G_SCALE << DEF_SELFTEST_ACCL_FULL_SCALE));
	thres =
	    accel_thres_mg * DEF_ST_SCALE /
	    (ACCL_G_SCALE << DEF_SELFTEST_ACCL_FULL_SCALE);
	printk("accel_bias=%d %d %d, thres=%ld(%d mg)\n", accel_bias[X],
	       accel_bias[Y], accel_bias[Z], thres, accel_thres_mg);
	for (j = 0; j < 3; j++) {
				    ...
				    int *reg_avg, int *st_avg)
{
	...
	g_z_sign = 1;
	ret_val = 0;
	test_setup.accl_sens[X] = (u32) (DEF_ST_SCALE * DEF_ST_PRECISION / fs);
	test_setup.accl_sens[Y] = (u32) (DEF_ST_SCALE * DEF_ST_PRECISION / fs);
	test_setup.accl_sens[Z] = (u32) (DEF_ST_SCALE * DEF_ST_PRECISION / fs);
	...
	for (j = 0; j < 3; j++) {
		...
		if (st_shift_prod[j]) {
			...
			if (st_shift_ratio[j] > DEF_ACCEL_ST_SHIFT_DELTA) {
				ret_val |= 1 << j;
				printk
				    ("%s, axis(%d) > DEF_ACCEL_ST_SHIFT_DELTA\n",
				     __func__, j);
			}
		} else {
			if (st_shift_cust[j] < DEF_ACCEL_ST_SHIFT_MIN * gravity) {
				ret_val |= 1 << j;
				printk
				    ("%s, axis(%d) < DEF_ACCEL_ST_SHIFT_MIN\n",
				     __func__, j);
			}
			if (st_shift_cust[j] > DEF_ACCEL_ST_SHIFT_MAX * gravity) {
				ret_val |= 1 << j;
				printk
				    ("%s, axis(%d) > DEF_ACCEL_ST_SHIFT_MAX\n",
				     __func__, j);
			}
		}
	}
	...
	if (ret_val == 0) {
		...
		for (i = 0; i < 3; i++) {
			if (abs(reg_avg[i]) > gyro_offset_max) {
				printk
				    ("GYRO:CRITERIA C: Gyro axis:%d > 20dps\n",
				     i);
				ret_val |= 1 << i;	/*Error condition*/
			}
		}
					 ...
					 int *reg_avg, int *st_avg)
{
	...
	if (otp_value_zero == 0) {
		...
		/* Self Test Pass/Fail Criteria B */
		gravity =
		    (u32) (DEF_ST_SCALE * DEF_ST_PRECISION /
...
#if INV_DO_TEST_DEBUG
			buf_debug[index_debug++] = (short)
			    be16_to_cpup((__be16 *) (&data[ind + 2 * 0]));
			buf_debug[index_debug++] = (short)
			    be16_to_cpup((__be16 *) (&data[ind + 2 * 1]));
			buf_debug[index_debug++] = (short)
			    be16_to_cpup((__be16 *) (&data[ind + 2 * 2]));
/*		printk("gyro, %d, %d, %d\n",
		(short)be16_to_cpup((__be16 *)(&data[ind + 2 * 0])),
		(short)be16_to_cpup((__be16 *)(&data[ind + 2 * 1])),
		(short)be16_to_cpup((__be16 *)(&data[ind + 2 * 2])));*/
	...
	printk("Accel offset data ST=%d, AX=%d, AY=%d, AZ=%d\n",
	       self_test_flag, accl_result[0], accl_result[1], accl_result[2]);

	printk("Gyro offset data ST=%d, GX=%d, GY=%d, GZ=%d\n",
	       self_test_flag, gyro_result[0], gyro_result[1], gyro_result[2]);
...
static int inv_check_compass_self_test(struct inv_mpu_iio_s *st)
{
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		...
		result =
		    inv_secondary_write(AK09911_REG_CNTL2,
		...
		result =
		    inv_secondary_write(REG_AKM_ST_CTRL, DATA_AKM_SELF_TEST);
		if (result)
			goto AKM_fail;
		/* set self test mode */
		...
		if (COMPASS_ID_AK8963 == st->plat_data.sec_slave_id) {
			result =
			    inv_secondary_read(REG_AKM8963_CNTL1, 1, &cntl);
			if (result)
				goto AKM_fail;
...
int inv_hw_self_test(struct inv_mpu_iio_s *st)
{
	...
	if (st->chip_config.is_asleep ||
	    ...
	    (!st->chip_config.gyro_enable) || (!st->chip_config.accl_enable)) {
		result = inv_power_up_self_test(st);
		if (result) {
			printk
			    ("Failed to run Self Test: Setup conditions fail\n");
			return result;
		}
	}
	mutex_lock(&st->mutex);
	...
	if (st->chip_type == INV_ITG3500) {
		...
		if (INV_MPU6050 == st->chip_type) {
			accel_result = !inv_check_accl_self_test(st,
								 ...
								 accl_bias_st);
			gyro_result =
			    !inv_check_6050_gyro_self_test(st,
							   gyro_bias_regular,
							   gyro_bias_st);
			...
			accel_result = !inv_check_6500_accl_self_test(st,
								      ...
								      accl_bias_st);
			gyro_result =
			    !inv_check_6500_gyro_self_test(st,
							   gyro_bias_regular,
							   gyro_bias_st);
		}
	}
	...
	printk("ST Result = %d\n",
	       (compass_result << DEF_ST_COMPASS_RESULT_SHIFT) | (accel_result
								  <<
								  DEF_ST_ACCEL_RESULT_SHIFT)
	       | gyro_result);
...
*/
	...
	accel_result |=
	    inv_check_accl_self_test_raw(st, accl_bias_regular,
					 ACCEL_BIAS_THRESHOLD_SMT);
	...
	gyro_result |=
	    inv_check_gyro_self_test_raw(st, gyro_bias_regular,
					 GYRO_BIAS_THRESHOLD_SMT);
	gyro_result = !gyro_result;
      test_fail:
	inv_recover_setting(st);
	return (compass_result << DEF_ST_COMPASS_RESULT_SHIFT) | (accel_result
								  <<
								  DEF_ST_ACCEL_RESULT_SHIFT)
	    | gyro_result;
...
static int inv_load_firmware(struct inv_mpu_iio_s *st, u8 *data, int size)
{
	int bank, write_size;
	int result;
	u16 memaddr;
	...
	for (bank = 0; size > 0; bank++, size -= write_size, data += write_size) {
...
static int inv_verify_firmware(struct inv_mpu_iio_s *st, u8 *data, int size)
{
	int bank, write_size;
	int result;
	u16 memaddr;
	...
	for (bank = 0; size > 0; bank++, size -= write_size, data += write_size) {
		...
		result = mpu_memory_read(st,
					 st->i2c_addr, memaddr, write_size,
					 firmware);
		if (result)
			return result;
...
static int inv_set_fifo_div(struct inv_mpu_iio_s *st, u16 fifoRate)
{
	u8 regs[2];
	int result = 0;
...
int inv_set_fifo_rate(struct inv_mpu_iio_s *st, u16 fifo_rate)
{
	...
	if (divider > DMP_MAX_DIVIDER) {
		...
		st->fifo_divider = (u8) (DMP_DEFAULT_FIFO_RATE / fifo_rate) - 1;
	} else {
...
static int inv_set_tap_interrupt_dmp(struct inv_mpu_iio_s *st, u8 on)
{
	int result;
	u8 regs[] = { 0 };
...
int inv_set_tap_threshold_dmp(struct inv_mpu_iio_s *st, u32 axis, u16 threshold)
{
...
static int inv_set_tap_axes_dmp(struct inv_mpu_iio_s *st, u32 axes)
{
...
static int inv_set_multiple_tap_time_dmp(struct inv_mpu_iio_s *st, u32 time)
{
...
int inv_set_accel_bias_dmp(struct inv_mpu_iio_s *st)
{
	...
	for (i = 0; i < 3; i++)
		accel_bias_body[i] = st->input_accel_bias[(inv_accel_orient >>
		    ...
		    tmp[i];
	for (i = 0; i < 3; i++)
		accel_bias_body[i] = inv_q30_mult(accel_sf, accel_bias_body[i]);
	for (i = 0; i < 3; i++)
...
static int inv_set_shake_reject_thresh_dmp(struct inv_mpu_iio_s *st, int thresh)
...
static int inv_set_shake_reject_time_dmp(struct inv_mpu_iio_s *st, u32 time)
{
...
static int inv_set_shake_reject_timeout_dmp(struct inv_mpu_iio_s *st, u32 time)
{
...
int inv_enable_tap_dmp(struct inv_mpu_iio_s *st, bool on)
{
	...
	result = inv_set_shake_reject_timeout_dmp(st, DMP_SHAKE_REJECT_TIMEOUT);
	return result;
}
