...
/*rocky add for accel lpf...*/
	result = inv_i2c_single_write(st, REG_6500_ACCEL_CONFIG2, 3);
	if (result)
		return result;
/*end*/
				   ...
				   const char *buf, size_t count)
{
	...
	if (st->chip_config.has_compass) {
		st->compass_divider = COMPASS_RATE_SCALE * fifo_rate / ONE_K_HZ;
...
int write_be32_key_to_mem(struct inv_mpu_iio_s *st, u32 data, int key)
{
	cpu_to_be32s(&data);
				  ...
				  const char *buf, size_t count)
{
	...
	case ATTR_DMP_TAP_THRESHOLD:{
			...
			for (i = 0; i < ARRAY_SIZE(ax); i++) {
				result =
				    inv_set_tap_threshold_dmp(st, ax[i], data);
				if (result)
					goto dmp_attr_store_fail;
			}
			st->tap.thresh = data;
...
static ssize_t get_mpu65xx_gyro_rawdata(struct device *dev,
...
static ssize_t get_mpu65xx_gsensor_rawdata(struct device *dev,
					   struct device_attribute *devattr,
					   char *buf)
{
...
#endif
#define IIO_CHAN_INFO_SHARED_BIT(type) BIT(type)
#define IIO_CHAN_INFO_SEPARATE_BIT(type) BIT(type)
...
#define INV_MPU_CHAN(_type, _channel2, _index)                \
	{                                                         \
		...
		.modified = 1,                                        \
		.channel2 = _channel2,                                \
		.info_mask_separate = IIO_CHAN_INFO_CALIBBIAS_SEPARATE_BIT,  \
		.scan_index = _index,                                 \
...
static IIO_DEVICE_ATTR(self_test, S_IRUGO, inv_attr_show, NULL, ATTR_SELF_TEST);
...
static IIO_DEVICE_ATTR(display_orientation_on, S_IRUGO | S_IWUSR,
		       inv_attr_show, inv_dmp_attr_store,
...
#ifdef INV_RAW_DATA_VIA_I2C
static DEVICE_ATTR(rawdata_gyro, S_IRUGO | S_IWUSR, get_mpu65xx_gyro_rawdata,
		   NULL);
static DEVICE_ATTR(rawdata_accel, S_IRUGO | S_IWUSR,
		   get_mpu65xx_gsensor_rawdata, NULL);
static DEVICE_ATTR(rawdata_compass, S_IRUGO | S_IWUSR,
		   get_mpu65xx_compass_rawdata, NULL);
...
#ifdef INV_RAW_DATA_VIA_I2C
					ARRAY_SIZE
					(inv_raw_i2c_mpu65xx_attributes) +
#endif
...
static int inv_mpu_resume(struct i2c_client *client)
{
	struct iio_dev *indio_dev = i2c_get_clientdata(client);
	struct inv_mpu_iio_s *st  = iio_priv(indio_dev);
	printk("inv_mpu_resume\n");
	inv_recover_setting(st);
	return 0;
}

static int inv_mpu_suspend(struct i2c_client *client, pm_message_t pm_message)
{
	printk("inv_mpu_suspend\n");
	return 0;
}

/**
 * Invensense_detect - Device detection callback for automatic device creation
 * return value:
 *                    = 0; success;
 *                    < 0; err
 */
static int invsens_detect(struct i2c_client *client,
			  struct i2c_board_info *info)
{
	struct i2c_adapter *adapter = client->adapter;
	int ret;
	int i = 0;
	pr_err("invsens_detect ====\n");
	...
	if (sensor_info.twi_id == adapter->nr) {
		while (i < 2) {
			...
			if (((ret&0x00FF) == MPU6500_ID) || ((ret&0xff) == 0x7d) || ((ret&0xff) == 0x76)) {
				pr_err("%s: MPU6500(i2c addr:0x%x) is detected!,id is :0x%x\n ", __func__, client->addr, ret&0x00FF);
				addr = client->addr;
				strlcpy(info->type, "mpu6500", I2C_NAME_SIZE);
				return 0;
			}
			pr_err("reg(0x75):0x%x, addr:0x%x. device is not find!\n", ret, client->addr);
		}
	} else {
		return -ENODEV;
	}
