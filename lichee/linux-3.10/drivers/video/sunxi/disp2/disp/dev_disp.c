...
static ssize_t disp_operate_3d_mode_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", operate_3d_mode);
}

static ssize_t disp_operate_3d_mode_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
	int err;
	unsigned long val;
	...
	err = strict_strtoul(buf, 10, &val);
	if (val > 6) {
		printk("Invalid value, 0~6 is expected!\n");
	} else {
		operate_3d_mode = val;
	}
	return count;
...
static DEVICE_ATTR(operate_3d_mode, S_IRUGO|S_IWUGO,
    disp_operate_3d_mode_show, disp_operate_3d_mode_store);
...
static int disp_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < dsi_clk_num; i++) {
		g_disp_drv.mclk[DISP_MOD_DSI0 + i] = of_clk_get(pdev->dev.of_node, counter);
