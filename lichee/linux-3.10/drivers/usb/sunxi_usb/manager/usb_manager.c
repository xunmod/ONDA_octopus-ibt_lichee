...
struct regulator *regulator_id_vbus = NULL;
...
static int sunxi_otg_manager_probe(struct platform_device *pdev)
{
	...
	struct device_node *usbc_np = NULL;
	char *regulator_id_vbus_name = NULL;
	int ret = -1;
	...
	/* usbc id_vbus */
	ret = of_property_read_string(usbc_np, KEY_USB_REGULATOR_ID_VBUS, &regulator_id_vbus_name);
	pr_info("get %s, regulator id_vbus: %s.\n", KEY_USB_REGULATOR_ID_VBUS, regulator_id_vbus_name);
	if (ret) {
		printk("get %s, regulator id_vbus is fail, %d\n", KEY_USB_REGULATOR_ID_VBUS, -ret);
		regulator_id_vbus_name = NULL;
	} else {
		if (!strcmp(regulator_id_vbus_name, "nocare")) {
			printk("get %s, regulator_ip_vbus is no nocare\n", KEY_USB_REGULATOR_ID_VBUS);
			regulator_id_vbus_name = NULL;
		}
	}

	if (regulator_id_vbus_name != NULL)
		regulator_id_vbus = regulator_get(NULL, regulator_id_vbus_name);
	if (regulator_id_vbus != NULL) {
		if(regulator_enable(regulator_id_vbus) < 0)
					DMSG_INFO("vcc-otg: regulator_id_vbus_enable fail\n");
		else
					DMSG_INFO("vcc-otg: regulator_id_vbus_enable OK\n");
	}
...
static int sunxi_otg_manager_remove(struct platform_device *pdev)
{
	...
	if (regulator_id_vbus != NULL)
		regulator_disable(regulator_id_vbus);
...
static int sunxi_otg_manager_resume(struct device *dev)
{
	atomic_set(&thread_suspend_flag, 0);
	if (regulator_id_vbus != NULL)
		regulator_enable(regulator_id_vbus);
	return 0;
}
