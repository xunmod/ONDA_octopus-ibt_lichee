...
thermal_sensor_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (indx = 0; indx < ths_data->sensor_cnt; indx++) {
		...
		if (!strcmp(attr->attr.name, buf)) {
			return sprintf(buf, "sensor%d located in %s, temp is = %ld\n",
					indx,
...
static long ths_driver_reg_to_temp(u32 reg_data, u16 id,
				   ...
				   struct temp_calculate_coefficent *para)
{
	...
	if (reg_data > para->down_limit.split_reg) {
		...
		if (ths_data->ths_driver_version == 1)
			t = reg_data * (para->nt_para[id].MUL_PARA) - para->nt_para[id].MINU_PARA;
		else
			t = para->nt_para[id].MINU_PARA - (reg_data * (para->nt_para[id].MUL_PARA));

		t = t / para->nt_para[id].DIV_PARA;
	} else if (reg_data < para->up_limit.split_reg) {
		thsprintk("NEXT:MUL_PARA=%d DIV_PARA=%d MINU_PARA=%d.\n",
			para->nt_para[id].MUL_PARA,
			para->nt_para[id].DIV_PARA,
			para->nt_para[id].MINU_PARA);
		t = ths_driver_reg_to_temp(reg_data, id, ths_data, para + 1);
	}

	return (long)t;
}

static u32 ths_driver_temp_to_reg(long temp,
			       struct temp_calculate_coefficent *para)
{
	s32 reg;

	reg = (para->nt_para[0].MINU_PARA -
		(temp * para->nt_para[0].DIV_PARA));
	reg = reg / para->nt_para[0].MUL_PARA;
	return (u32) reg;
}


long ths_driver_get_temp(struct sunxi_ths_data *ths_data, int id)
{
	u32 reg_data;
	u32 reg_flag = 0;
	int ths_drv_ver;
...
/*
 *the new test method add the enviroment temp, and write the efuse value after
 *sub the enviroment temp
 */
static void write_new_efuse_to_reg(struct sunxi_ths_data *ths_data,
					  int cal_reg_num,
					  ...
					  u32 *ths_cal_data)
{
	...
	u32 write_value = 0;
	u16 *ths_16bit_cal_data;

	if (*ths_cal_data == 0)
		pr_info("%s:reading environment_temp is zero\n",
				__func__);

	environment_temp = (u16)*ths_cal_data;
	ths_16bit_cal_data = (u16 *)(ths_cal_data + 1);

	for (i = 0, calcu_times = 0; i < ths_data->sensor_cnt; i++) {
		if (*(ths_16bit_cal_data + i) == 0) {
		...
		if (calcu_times == 0) {
			...
			if ((i + 1) < ths_data->sensor_cnt) {
				write_value = (u32)((*(ths_16bit_cal_data + i) - environment_temp) + 0x800);
			} else {
				write_value = (u32)((*(ths_16bit_cal_data + i) - environment_temp) + 0x800);
		...
		if (calcu_times == 1) {
			calcu_times = 0;
			write_value |= (((u32)(*(ths_16bit_cal_data + i) - environment_temp) + 0x800) << 16);
...
static void write_old_efuse_to_reg(struct sunxi_ths_data *ths_data,
					  int cal_reg_num,
					  struct thermal_reg *reg,
					  u32 *ths_cal_data)
{
	...
	for (i = 0; i < cal_reg_num; i++) {
		...
		sunxi_ths_writel(*(ths_cal_data + i), ths_data->base_addr +
						reg->offset + i * 4);
		reg_data = sunxi_ths_readl(ths_data->base_addr + reg->offset + i * 4);
					  ...
					  struct thermal_reg *reg)
{
	...
	if (ret) {
		...
		if (ths_drv_ver == 1 || ths_drv_ver == 2) {
			write_old_efuse_to_reg(ths_data, cal_reg_num, reg,
					       ths_cal_data);
		} else if (ths_drv_ver == 3) {
			write_new_efuse_to_reg(ths_data, cal_reg_num, reg,
					       ths_cal_data);
		}
	}
	kfree(ths_cal_data);
					...
					struct temp_calculate_coefficent *cpara)
{
	...
	reg_value = ths_driver_temp_to_reg(ths_data->shut_temp, cpara);
...
static void ths_driver_init_new_shut_reg(struct sunxi_ths_data *ths_data,
					...
					struct temp_calculate_coefficent *cpara)
{
	u32 reg_value, reg_value_store;
	int do_num, i, j;

	do_num = ths_data->sensor_cnt / REG_HAVE_SHUT_TEMP_NUM;
	for (i = 0; i < do_num; i++) {
		for (j = 0; j < REG_HAVE_SHUT_TEMP_NUM; j++) {
			reg_value_store =
				ths_driver_temp_to_reg(ths_data->shut_temp,
						       cpara);
			reg_value_store = reg_value_store << (j * 16);
			reg_value = reg_value_store | reg_value;
		}
		sunxi_ths_writel(reg_value, ths_data->base_addr +
				  reg->offset + i * 4);
		thsprintk("THS_SHUTDOWN_CTRL_REG_%d = 0x%x\n",
			i, sunxi_ths_readl(ths_data->base_addr +
				reg->offset + i * 4));
		reg_value = 0;
	}
	/*
	 *if total_sensor_num is not even number,just move on 4 offset
	 *write once.
	 */
	if (ths_data->sensor_cnt % REG_HAVE_SHUT_TEMP_NUM != 0) {
		reg_value =
			ths_driver_temp_to_reg(ths_data->shut_temp, cpara);
		sunxi_ths_writel(reg_value, ths_data->base_addr +
				  reg->offset + i * 4);
		thsprintk("THS_SHUTDOWN_CTRL_REG_%d = 0x%x\n",
			i, sunxi_ths_readl(ths_data->base_addr +
