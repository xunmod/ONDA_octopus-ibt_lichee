			     ...
			     unsigned long *config)
{
	...
	case SUNXI_PINCFG_TYPE_DRV:
		val = pinctrl_readl_reg(pctl->membase + sunxi_dlevel_reg(pin));
	...
	case SUNXI_PINCFG_TYPE_PUD:
		val = pinctrl_readl_reg(pctl->membase + sunxi_pull_reg(pin));
	...
	case SUNXI_PINCFG_TYPE_DAT:
		val = pinctrl_readl_reg(pctl->membase + sunxi_data_reg(pin));
	...
	case SUNXI_PINCFG_TYPE_FUNC:
		val = pinctrl_readl_reg(pctl->membase + sunxi_mux_reg(pin));
			     ...
			     unsigned num_configs)
{
	...
	case SUNXI_PINCFG_TYPE_DRV:
		...
		pinctrl_write_reg((val & ~mask) | dlevel << sunxi_dlevel_offset(pin),
	...
	case SUNXI_PINCFG_TYPE_PUD:
		...
		pinctrl_write_reg((val & ~mask) | pull << sunxi_pull_offset(pin),
	...
	case SUNXI_PINCFG_TYPE_DAT:
		...
		pinctrl_write_reg((val & ~mask) | data << sunxi_data_offset(pin),
	...
	case SUNXI_PINCFG_TYPE_FUNC:
		...
		pinctrl_write_reg((val & ~mask) | func << sunxi_mux_offset(pin),
				 ...
				 unsigned num_configs)
{
	...
	for (i = 0; i < num_configs; i++) {
		...
		case PIN_CONFIG_BIAS_PULL_PIN_DEFAULT:
			val = pinctrl_readl_reg(pctl->membase + sunxi_pull_reg(pin));
			mask = PULL_PINS_MASK << sunxi_pull_offset(pin);
			pinctrl_write_reg((val & ~mask) | 0 << sunxi_pull_offset(pin),
				pctl->membase + sunxi_pull_reg(pin));
			break;
		case PIN_CONFIG_BIAS_PULL_UP:
			val = pinctrl_readl_reg(pctl->membase + sunxi_pull_reg(pin));
			mask = PULL_PINS_MASK << sunxi_pull_offset(pin);
			pinctrl_write_reg((val & ~mask) | 1 << sunxi_pull_offset(pin),
				pctl->membase + sunxi_pull_reg(pin));
			break;
		case PIN_CONFIG_BIAS_PULL_DOWN:
			val = pinctrl_readl_reg(pctl->membase + sunxi_pull_reg(pin));
			mask = PULL_PINS_MASK << sunxi_pull_offset(pin);
			pinctrl_write_reg((val & ~mask) | 2 << sunxi_pull_offset(pin),
				pctl->membase + sunxi_pull_reg(pin));
			break;
				 ...
				 bool enable)
{
	...
	pinctrl_write_reg(val, pctl->membase + sunxi_mux_reg(pin));
...
static int sunxi_pinctrl_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	u32 val = (pinctrl_readl_reg(pctl->membase + reg) >> index) & DATA_PINS_MASK;
				...
				unsigned offset, int value)
{
	...
	regval = pinctrl_readl_reg(pctl->membase + reg);
	...
	pinctrl_write_reg(regval, pctl->membase + reg);
					...
					unsigned offset, unsigned value)
{
	...
	spin_lock_irqsave(&pctl->lock, flags);
	reg_val = pinctrl_readl_reg(pctl->membase + reg);
	...
	pinctrl_write_reg(reg_val, pctl->membase + reg);
	spin_unlock_irqrestore(&pctl->lock, flags);
...
static int sunxi_pinctrl_irq_set_type(struct irq_data *d, unsigned int type)
{
	...
	pinctrl_write_reg(regval | (mode << index), pctl->membase + reg);
...
static void sunxi_pinctrl_irq_ack(struct irq_data *d)
{
	...
	/* Clear the IRQ */
	pinctrl_write_reg(1 << status_idx, pctl->membase + status_reg);
}
...
static void sunxi_pinctrl_irq_mask(struct irq_data *d)
{
	...
	/* Mask the IRQ */
	val = pinctrl_readl_reg(pctl->membase + reg);
	pinctrl_write_reg(val & ~(1 << idx), pctl->membase + reg);
		       ...
		       const struct sunxi_pinctrl_desc *desc)
{
	...
	for (i = 0; i < pctl->desc->irq_banks; i++) {
		...
		pinctrl_write_reg(0, pctl->membase + sunxi_irq_ctrl_reg_from_bank(i));
		pinctrl_write_reg(0xffffffff, pctl->membase + sunxi_irq_status_reg_from_bank(i));
		if(pctl->desc->pin_base >= PL_BASE){
