...
#if defined(CONFIG_SUNXI_SMC) || defined(CONFIG_SUNXI_TRUSTZONE)
...
void sid_rd_soc_secure_status(void)
{
...
#ifdef CONFIG_ARCH_SUN8IW6
   {
	   ...
	   reserved_value = readl((void __iomem *)0xf0044000);
	   if (reserved_value == reserved_data)
		   reserved_data = 0xa5a5;

	   writel(reserved_data, (void __iomem *)0xf0044000);
	   reg_val = readl((void __iomem *)0xf0044000);
	   if (reg_val != reserved_data) {
		   /* all 0, secure is enable */
		   sunxi_soc_secure = 1;
	   } else {
		   writel(reserved_value, (void __iomem *)0xf0044000);
	   }
   }
#else
