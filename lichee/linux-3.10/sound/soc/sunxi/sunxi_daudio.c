				...
				int clk_id, int clk_div)
{
	...
	if (sunxi_daudio->pdata->pcm_lsb_first) {
		/* lsb first */
		regmap_update_bits(sunxi_daudio->regmap,
			SUNXI_DAUDIO_FMT1, 1<<TX_MLS, 0);
		regmap_update_bits(sunxi_daudio->regmap,
			SUNXI_DAUDIO_FMT1, 1<<RX_MLS, 0);
	} else {
		/* msb first */
		regmap_update_bits(sunxi_daudio->regmap,
			SUNXI_DAUDIO_FMT1, 1<<TX_MLS, 1);
		regmap_update_bits(sunxi_daudio->regmap,
			SUNXI_DAUDIO_FMT1, 1<<RX_MLS, 1);
	}

	/* signext */
	regmap_update_bits(sunxi_daudio->regmap, SUNXI_DAUDIO_FMT1, 3<<SEXT, 0);
#endif
...
static int sunxi_daudio_dev_probe(struct platform_device *pdev)
{
	...
	case	SUNXI_DAUDIO_EXTERNAL_TYPE:
		...
		if (ret < 0) {
			...
			dev_dbg(&pdev->dev, "daudio_master:%d\n", temp_val);
		}
		...
		if (ret < 0) {
			...
			dev_dbg(&pdev->dev, "pcm_lrck_period:%d\n", temp_val);
		}
		...
		if (ret < 0) {
			...
			dev_dbg(&pdev->dev, "slot_width_select:%d\n", temp_val);
		}

		ret = of_property_read_u32(np, "pcm_lsb_first", &temp_val);
		if (ret < 0) {
			dev_warn(&pdev->dev, "pcm_lsb_first config missing or invalid\n");
			sunxi_daudio->pdata->pcm_lsb_first = 1;
		} else {
			sunxi_daudio->pdata->pcm_lsb_first = temp_val;
