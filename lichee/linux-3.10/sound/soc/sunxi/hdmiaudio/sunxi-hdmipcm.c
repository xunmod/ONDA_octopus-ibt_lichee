...
int sun8iw6_hdmi_transfer_format_61937_to_60958(int *out, short *temp, int samples)
{
	int ret = 0;
	int i;
	static int numtotal;
	...
	samples >>= 1;
	sun8iw6_head.head0.other = 0;
	sun8iw6_head.head0.B = 1;
	sun8iw6_head.head0.P = 0;
	sun8iw6_head.head0.C = 0;
	sun8iw6_head.head0.U = 0;
	sun8iw6_head.head0.V = 1;
	...
	for (i = 0; i < samples; i++, numtotal++) {
		if ((numtotal%384 == 0) || (numtotal%384 == 1)) {
			...
			sun8iw6_head.head0.B = 0;
		}
		sun8iw6_head.head0.C = channel_status[(numtotal%384)/2];
		...
		sun8iw6_head.head0.P = w1.bits.bit15 ^ w1.bits.bit14 ^ w1.bits.bit13 ^ w1.bits.bit12
	 ...
	 snd_pcm_uframes_t hwoff, void __user *buf, snd_pcm_uframes_t frames)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (raw_flag > 1) {
			...
			sun8iw6_hdmi_transfer_format_61937_to_60958((int *)hdmihw_area, (short *)hwbuf, frames_to_bytes(runtime, frames));
		}
#ifdef AUDIO_KARAOKE
...
static struct platform_driver sunxi_hdmiaudio_pcm_driver = {
	...
	.remove = sunxi_hdmiaudio_pcm_remove,
	.driver = {
		.name 	= "sunxi-hdmiaudio-pcm-audio",
		.owner 	= THIS_MODULE,
	},
...
static int __init sunxi_soc_platform_hdmiaudio_init(void)
{
	...
	err = platform_device_register(&sunxi_hdmiaudio_pcm_device);
	if (err < 0) {
		return err;
	}
