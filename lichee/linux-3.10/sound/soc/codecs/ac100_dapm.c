...
static int hook_flag1;
static int hook_flag2;
...
static void set_configuration(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, MOD_CLK_ENA,
		(0x1<<MOD_CLK_ADC_DIG), (0x1<<MOD_CLK_ADC_DIG));
	snd_soc_update_bits(codec, MOD_RST_CTRL,
		(0x1<<MOD_RESET_ADC_DIG), (0x1<<MOD_RESET_ADC_DIG));

	snd_soc_update_bits(codec, SPKOUT_CTRL,
		(0x1<<RSPK_EN), (0x1<<RSPK_EN));
	snd_soc_update_bits(codec, SPKOUT_CTRL,
		(0x1<<LSPK_EN), (0x1<<LSPK_EN));
#endif
...
#ifdef CONFIG_SND_SUNXI_SOC_DAUDIO0_KARAOKE_MACHINE
	SOC_SINGLE("AIF1 ADCR Switch Duplicate",
		AIF1_MXR_SRC, AIF1_AD0R_ADCR_MXR, 0x1, 0),
	SOC_SINGLE("DACR ADCR Switch Duplicate",
		DAC_MXR_SRC, DACR_MXR_ADCR, 1, 0),
#endif
...
static const struct snd_soc_dapm_widget ac100_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX("AIF1OUT0L Mux",
		AIF1_ADCDAT_CTRL, 15, 0, &aif1out0l_mux),
	SND_SOC_DAPM_MUX("AIF1OUT0R Mux",
		AIF1_ADCDAT_CTRL, 14, 0, &aif1out0r_mux),

	SND_SOC_DAPM_MUX("AIF1OUT1L Mux",
		AIF1_ADCDAT_CTRL, 13, 0, &aif1out1l_mux),
	SND_SOC_DAPM_MUX("AIF1OUT1R Mux",
		AIF1_ADCDAT_CTRL, 12, 0, &aif1out1r_mux),

	SND_SOC_DAPM_MUX("AIF1IN0L Mux",
		AIF1_DACDAT_CTRL, 15, 0, &aif1in0l_mux),
	SND_SOC_DAPM_MUX("AIF1IN0R Mux",
		AIF1_DACDAT_CTRL, 14, 0, &aif1in0r_mux),

	SND_SOC_DAPM_MUX("AIF1IN1L Mux",
		AIF1_DACDAT_CTRL, 13, 0, &aif1in1l_mux),
	SND_SOC_DAPM_MUX("AIF1IN1R Mux",
		AIF1_DACDAT_CTRL, 12, 0, &aif1in1r_mux),
	...
	SND_SOC_DAPM_MUX("SPK_R Mux",
		SND_SOC_NOPM, 0, 0, &ac100_rspks_func_controls),
	SND_SOC_DAPM_MUX("SPK_L Mux",
		SND_SOC_NOPM, 0, 0, &ac100_lspks_func_controls),
#else
	SND_SOC_DAPM_MUX("SPK_R Mux",
	...
	SND_SOC_DAPM_MUX("AIF2OUTL Mux",
		AIF2_ADCDAT_CTRL, 15, 0, &aif2outl_mux),
	SND_SOC_DAPM_MUX("AIF2OUTR Mux",
		AIF2_ADCDAT_CTRL, 14, 0, &aif2outr_mux),
	...
	SND_SOC_DAPM_MIXER_E("LEFT ADC input Mixer", ADC_APC_CTRL, ADCLEN, 0,
		ac100_ladcmix_controls, ARRAY_SIZE(ac100_ladcmix_controls),
	...
	SND_SOC_DAPM_MIXER_E("RIGHT ADC input Mixer", ADC_APC_CTRL, ADCREN, 0,
		ac100_radcmix_controls, ARRAY_SIZE(ac100_radcmix_controls),
		late_enable_adc, SND_SOC_DAPM_PRE_PMU|SND_SOC_DAPM_POST_PMD),
#else
...
static void switch_hw_config(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, HMIC_CTRL1,
		(0x1<<HMIC_PULLOUT_IRQ), (0x1<<HMIC_PULLOUT_IRQ));
	snd_soc_update_bits(codec, HMIC_CTRL1,
		(0x1<<HMIC_PLUGIN_IRQ), (0x1<<HMIC_PLUGIN_IRQ));

	/*Hmic KeyUp/key down Irq Enable*/
	snd_soc_update_bits(codec, HMIC_CTRL1,
		(0x1<<HMIC_KEYDOWN_IRQ), (0x1<<HMIC_KEYDOWN_IRQ));
	snd_soc_update_bits(codec, HMIC_CTRL1,
		(0x1<<HMIC_KEYUP_IRQ), (0x1<<HMIC_KEYUP_IRQ));
...
static void clear_codec_irq_work(struct work_struct *work)
{
	int reg_val = 0;
	struct ac100_priv *ac100 =
	    container_of(work, struct ac100_priv, clear_codec_irq);
...
static void earphone_switch_work(struct work_struct *work)
{
	...
	if ((tmp >= 0x10) && (ac100->mode == FOUR_HEADPHONE_PLUGIN)) {
		tmp = snd_soc_read(codec, HMIC_STS);
		tmp = (tmp>>HMIC_DATA);
		tmp &= 0x1f;
		if (tmp >= 0x15) {
			...
			tmp = (tmp>>HMIC_DATA);
			tmp &= 0x1f;
			if (((tmp < 0xb && tmp >= 0x1) || tmp >= 0x19) &&
				(reset_flag == 0)) {
				input_report_key(ac100->key,
					KEY_HEADSETHOOK, 1);
			...
			if (reset_flag)
				reset_flag--;
		} else if (tmp < 0x15 && tmp >= 0x13) {
			msleep(80);
			tmp = snd_soc_read(codec, HMIC_STS);
			tmp = (tmp>>HMIC_DATA);
			tmp &= 0x1f;
			if (tmp < 0x15 && tmp >= 0x13 &&
				(reset_flag == 0)) {
				KEY_VOLUME_FLAG = 1;
			...
			if (reset_flag)
				reset_flag--;
		} else if (tmp < 0x13 && tmp >= 0x11) {
			msleep(80);
			tmp = snd_soc_read(codec, HMIC_STS);
			tmp = (tmp>>HMIC_DATA);
			tmp &= 0x1f;
			if (tmp < 0x13 && tmp >= 0x11 &&
				(reset_flag == 0)) {
				KEY_VOLUME_FLAG = 1;
				input_report_key(ac100->key,
					KEY_VOLUMEDOWN, 1);
		...
		while (irq_flag == 0) {
			...
			if (ac100->check_count_sum <= HEADSET_CHECKCOUNT_SUM) {
				...
				if (temp_value[ac100->check_count - 2]
					>= 0x15) {
					...
					ac100->check_count - 2] >= 0x1 &&
					temp_value[ac100->check_count - 2]
					< 0x15) {
					ac100->mode = FOUR_HEADPHONE_PLUGIN;
					ac100->state = 1;
					switch_status_update(ac100);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (flag) {
		...
		num = val&0xff;
		printk(KERN_INFO"\n");
		printk(KERN_INFO"read:start add:0x%x,count:0x%x\n", reg, num);
	...
	struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_INFO"echo flag|reg|val > ac100\n");
	printk(KERN_INFO"eg read addr=0x06,count 0x10:echo 0610 >ac100\n");
	printk(KERN_INFO"eg write val:0x13fe to 0x06 :echo 10613fe > ac100\n");
	printk(KERN_INFO"------------------------------------------------\n");
...
static int ac100_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (!ac100->key) {
		pr_err("[AC100] not enough memory for input device\n");
		ret = -ENOMEM;
	...
	if (ret) {
		pr_err("[AC100] input_register_device failed\n");
	...
	*/
	...
	ret = gpio_set_debounce(hsgpio.gpio, 1);
	if (ret) {
		pr_warn("[AC100]gpio_set_debouncefailed(ret:%d)\n", ret);
		return -EINVAL;
	}
...
static int ac100_codec_suspend(struct snd_soc_codec *codec)
{
	...
	sunxi_gpio_to_name(hsgpio.gpio, pin_name);
...
static int ac100_probe(struct platform_device *pdev)
{
	...
	hsgpio.gpio = of_get_named_gpio_flags(node, "gpio-hs", 0,
		(enum of_gpio_flags *)&config);
	if (!gpio_is_valid(hsgpio.gpio)) {
		pr_err("failed to get gpio-hs gpio from dts,hsgpio:%d\n",
			hsgpio.gpio);
		hsgpio.used = 0;
	} else {
		ret = devm_gpio_request(&pdev->dev, hsgpio.gpio, "HEADSET");
		if (ret) {
			hsgpio.used = 0;
			pr_err("failed to request gpio-hs gpio\n");
		} else {
			hsgpio.used = 1;
			pr_debug("set spkgpio ok(hsgpio:%d)\n", hsgpio.gpio);
		}
	}
