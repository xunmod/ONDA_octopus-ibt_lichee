...
static struct hwspinlock *rtc_hw_lock;

static void rtc_hwspin_lock_init(void)
{
	rtc_hw_lock = hwspin_lock_request_specific(SUNXI_RTC_HWSPINLOCK);
	if (!rtc_hw_lock)
...
{
	return hwspin_lock_timeout_irqsave(rtc_hw_lock, timeout, flags);
}

static int rtc_hwspin_unlock(unsigned long *flags)
{
	hwspin_unlock_irqrestore(rtc_hw_lock, flags);
	return 0;
}
#else
static int rtc_hwspin_lock_timeout(unsigned int timeout, unsigned long *flags)
...
static int rtc_gpreg_write(unsigned int regnum, unsigned int data)
{
	...
	if (!rtc_hwspin_lock_timeout(100, &hwflags)) {
		...
		sunxi_smc_writel(temp, SUNXI_RTC_GPREG_BASE);
		temp |= 1 << 31;
		sunxi_smc_writel(temp, SUNXI_RTC_GPREG_BASE);
		temp &= ~(1 << 31);
		sunxi_smc_writel(temp, SUNXI_RTC_GPREG_BASE);
#else
		sunxi_smc_writel(data, SUNXI_RTC_GPREG_BASE + 0x4 * regnum);
#endif
...
{
	char *reboot_args = (char *)data;
	switch (code) {
	case SYS_POWER_OFF:
		sunxi_sys_power_off(reboot_args);
		break;
	case SYS_RESTART:
		sunxi_sys_restart(reboot_args);
		break;
	}
