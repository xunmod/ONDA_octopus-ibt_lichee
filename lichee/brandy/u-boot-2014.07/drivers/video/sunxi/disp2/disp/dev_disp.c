...
#if defined(DISPLAY_WITHOUT_CLK_DRV)
int sys_cfg_lcd_dclk = 0;
#endif
...
#if defined(DISPLAY_WITHOUT_CLK_DRV)
static void drv_disp_clk_cfg(void)
{
	int ret = 0;
	int value = 0;

	ret = disp_sys_script_get_item(FDT_LCD0_PATH, "lcd_dclk_freq", &value, 1);
	if (ret == 1)
		sys_cfg_lcd_dclk = value;

	writel(0x80002300,0x01c20010);/*PLL_VIDEO0_CTRL:840M*/
	writel(0x80042400,0x01c20048);/*PLL_DE_CTRL:432M*/
	udelay(100);
	writel(0x80000000,0x01c20118);/*TCON0_CLK*/
}
#endif
