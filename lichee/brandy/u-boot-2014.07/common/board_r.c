...
static int sunxi_burn_key(void)
{
	...
	if(gd->vbus_status != SUNXI_VBUS_EXIST)	//vbus is not exist,return
	{
		printf("%s without usb!!skip burn key func\n",__func__);
		return 0;
	}
...
static int display_for_hdcp (void)
{
	...
	{
		/* if hdcp exist,display open should be call after flash init */
	...
	board_display_device_open();
	board_display_layer_request();
	tick_printf("%s display init end\n",__func__);
	return 0;
}
#endif
