	...
	struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		sunxi_slave_id(DRQDST_TDM_TX, DRQSRC_SDRAM);
#else
		slave_config.slave_id =
		sunxi_slave_id(dmap->dma_drq_type_num, DRQSRC_SDRAM);
#endif
...
#if defined CONFIG_ARCH_SUN8IW6
		slave_config.slave_id =
			sunxi_slave_id(DRQDST_SDRAM, DRQDST_TDMRX);
#else
		slave_config.slave_id =
			sunxi_slave_id(DRQDST_SDRAM, dmap->dma_drq_type_num);
#endif
	}
