...
#endif
void acquire_ve_idle(struct iomap_para *addrs)
{
	unsigned int need_acquire_ve_idle;
	unsigned long ve_ic_ver_old_reg;
	unsigned long ve_ic_ver_dec_reg;
	unsigned long ve_ic_ver_enc_reg;
	unsigned int ic_old_value;
	unsigned int ic_dec_value;
	unsigned int ic_enc_value;

	ve_ic_ver_old_reg = (unsigned long)(addrs->regs_macc + 0xf0);
	ve_ic_ver_dec_reg = (unsigned long)(addrs->regs_macc + 0xe0);
	ve_ic_ver_enc_reg = (unsigned long)(addrs->regs_macc + 0xe4);
	ic_old_value = readl((void *)ve_ic_ver_old_reg);
	ic_dec_value = readl((void *)ve_ic_ver_dec_reg);
	ic_enc_value = readl((void *)ve_ic_ver_enc_reg);
	if ((!ic_old_value) && (ic_dec_value == 0x31010) && (ic_enc_value == 0x12010))
		need_acquire_ve_idle = 1;
	else
		need_acquire_ve_idle = 0;

	if (need_acquire_ve_idle) {
		unsigned int i = 0;
		unsigned int dw_val = 0;
		unsigned long ve_sync_reg = 0;
		ve_sync_reg = (unsigned long)(addrs->regs_macc + 0x04);
		dw_val = readl((void *)ve_sync_reg) & (0x300);
		while (dw_val != 0x300) {
			i++;
			if (i > 0x400000) {
				cedar_ve_printk(KERN_WARNING, "wait ve mem sync idle bit too long, reset anyway after 10 ms.");
...
static int cedardev_init(struct platform_device *pdev)
{
...
#else
	/*VE_SRAM mapping to AC320*/
	val = sunxi_smc_readl((void *)cedar_devp->sram_bass_vir);
	val &= 0x80000000;
	sunxi_smc_writel(val, (void *)cedar_devp->sram_bass_vir);
	...
	/*clear bootmode bit for give sram to ve*/
	val = sunxi_smc_readl((void *)(cedar_devp->sram_bass_vir + 1));
	val &= 0xfeffffff;
	sunxi_smc_writel(val, (void *)(cedar_devp->sram_bass_vir + 1));
#endif
