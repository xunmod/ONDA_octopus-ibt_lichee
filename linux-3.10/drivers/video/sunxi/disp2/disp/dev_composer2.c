...
typedef struct hwc_sync_timeline {
	...
	unsigned int timeline_next_num;
	...
	unsigned int disp, unsigned int id, unsigned int count)
{
	...
	if ((DISP_SCREEN_NUM > disp) && (HWC_SYNC_TIMELINE_NUM > id)) {
		...
		if (0 != tl->inited) {
			...
			tl->timeline_next_num += count; /* fixme: maybe should add lock */
			pt = sw_sync_pt_create(tl->release_timeline, tl->timeline_next_num);
			if (pt == NULL) {
				ret = -ENOMEM;
				goto err;
			}
			...
			snprintf(fence_name, sizeof(fence_name) / sizeof(fence_name[0]),
					"TL%u_%u_Frame%x", disp, id, tl->timeline_next_num);
			...
			if (!disp_device_is_enable(disp)) {
				...
				tl->posted_frame_num = tl->timeline_next_num;
				mutex_lock(&(priv->posted_mlock));
				update_sync_timeline_locked(tl);
				mutex_unlock(&(priv->posted_mlock));
			}
...
int submit_layer_fence(unsigned int disp, unsigned int id, int fence_fd)
{
	...
	if ((DISP_SCREEN_NUM > disp) && (HWC_SYNC_TIMELINE_NUM > id)) {
		...
		printk("hwc give us bad disp %d id %d.\n", disp, id);
		return -1;
	}
	...
	if (pt != NULL) {
		tl->commit_frame_num = ((struct sw_sync_pt *)pt)->value;
	} else {
		tl->commit_frame_num++;
	}
#endif
...
static int composer2_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	if (DISP_HWC_COMMIT == cmd) {
		...
		case HWC_GET_CLK_RATE:
			...
			break;
		case HWC_SUBMIT_LAYER_FENCE:
			ret = submit_layer_fence((unsigned int)ubuffer[0],
				(unsigned int)ubuffer[2], (int)ubuffer[3]);
			break;
		default:
