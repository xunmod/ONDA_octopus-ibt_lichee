...
__s32 dsi_dcs_wr_4para(__u32 sel, __u8 cmd, __u8 para1, __u8 para2,
...
__s32 dsi_dcs_wr_memory(__u32 sel, __u32 *p_data, __u32 length)
{
	...
	while (tx_cntr) {
		...
		if (start) {
			dsi_dcs_wr(sel, DSI_DCS_WRITE_MEMORY_START,
					para, tx_num * 3);
			start = 0;
		} else {
			dsi_dcs_wr(sel, DSI_DCS_WRITE_MEMORY_CONTINUE,
					para, tx_num * 3);
		}
	}
	return 0;
...
__s32 dsi_dphy_cfg_0data(__u32 sel, __u32 code)
{
	...
	dsi_delay_us(1);
	dsi_dev[sel]->dsi_phy_ctl3.dwval = ((dsi_dev[sel]->dsi_phy_ctl3.dwval)
						& (~0x00ff0000)) | (code << 16);
	dsi_delay_us(1);
...
static __s32 dsi_dphy_cfg(__u32 sel, disp_panel_para *panel)
{
	...
	*/
	...
	if (LCD_DSI_IF_VIDEO_MODE == mode || LCD_DSI_IF_BURST_MODE == mode) {
		dsi_dphy_cfg_hsfrq(sel, dclk
					* dsi_bits_per_pixel[format] / lane);
	...
	dsi_delay_ms(1);
	dsi_dev[sel]->dsi_phy_ctl1.dwval = ((dsi_dev[sel]->dsi_phy_ctl1.dwval)
					& (~0xff)) | 10;
...
static __s32 dsi_basic_cfg(__u32 sel, disp_panel_para *panel)
{
	...
	if (LCD_DSI_IF_VIDEO_MODE == mode || LCD_DSI_IF_BURST_MODE == mode) {
		dsi_dev[sel]->dsi_ctl2.dwval = ((dclk
						* dsi_bits_per_pixel[format] +
						4 * lane * 10)
						/ (8 * lane * 10)) & 0xff;
		dsi_delay_ms(100);
		dsi_dev[sel]->dsi_to_ctl0.dwval = ((dclk *
						dsi_bits_per_pixel[format] + 4
						* lane * 10)
						/(8 * lane * 10)) & 0xff;
	} else {
		dsi_dev[sel]->dsi_ctl2.dwval = ((dclk * 6
			* dsi_bits_per_pixel[format] + 4 * lane * 10)
				/ (8 * lane * 10)) & 0xff;
		dsi_delay_ms(100);
		dsi_dev[sel]->dsi_to_ctl0.dwval = ((dclk * 6
			* dsi_bits_per_pixel[format] + 4 * lane * 10)
			/ (8 * lane * 10)) & 0xff;
	}
...
static __s32 dsi_packet_cfg(__u32 sel, disp_panel_para *panel)
{
	if (panel->lcd_dsi_if == LCD_DSI_IF_COMMAND_MODE) {
		...
		dsi_dev[sel]->dsi_dbi_ctl1.bits.allowed_cmd_size = x *
					dsi_bits_per_pixel[format]/8+1;
		dsi_dev[sel]->dsi_dbi_ctl1.bits.wr_cmd_size = x *
					dsi_bits_per_pixel[format]/8+1;
		...
		dsi_dev[sel]->dsi_vid_tim3.bits.hsa = hspw *
					dsi_bits_per_pixel[format]/(8 * lane);
		dsi_dev[sel]->dsi_vid_tim3.bits.hbp = (hbp-hspw)
					* dsi_bits_per_pixel[format]/(8 * lane);
		dsi_dev[sel]->dsi_vid_tim2.bits.ht = ht
					* dsi_bits_per_pixel[format]/(8 * lane);
