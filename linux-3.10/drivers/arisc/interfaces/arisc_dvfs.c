...
int arisc_dvfs_cfg_vf_table(void)
{
	...
	for (cluster = 0; cluster < 1; cluster++) {
		...
		dvfs_sub_np = of_find_compatible_node(dvfs_main_np, NULL, vf_table_main_key);
		if (!dvfs_sub_np) {
			ARISC_ERR("No %s node found\n", vf_table_main_key);
			return -ENODEV;
		}

		if (of_property_read_u32(dvfs_sub_np, vf_table_sub_key, &vf_table_size[cluster])) {
		...
		for (index = 0; index < vf_table_size[cluster]; index++) {
			...
			if (of_property_read_u32(dvfs_sub_np, vf_table_sub_key, &arisc_vf_table[cluster][index].freq)) {
				ARISC_ERR("get %s failed\n", vf_table_sub_key);
				return -EINVAL;
			}

			ARISC_INF("%s: freq [%s-%d=%d]\n", __func__, vf_table_sub_key, index, arisc_vf_table[cluster][index].freq);

			sprintf(vf_table_sub_key, "%c_LV%d_volt", cluster_name[cluster], index + 1);
			if (of_property_read_u32(dvfs_sub_np, vf_table_sub_key, &arisc_vf_table[cluster][index].voltage)) {
				ARISC_ERR("get %s failed\n", vf_table_sub_key);
				return -EINVAL;
			}

			ARISC_INF("%s: volt [%s-%d=%d]\n", __func__, vf_table_sub_key, index, arisc_vf_table[cluster][index].voltage);
		}
	}
