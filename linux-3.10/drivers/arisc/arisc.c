...
static int sunxi_arisc_parse_cfg(struct platform_device *pdev)
{
	...
	/* parse s_cpucfg node */
	arisc_cfg.scpucfg.np = of_find_compatible_node(NULL, NULL, "allwinner,s_scpucfg");
	if (IS_ERR(arisc_cfg.scpucfg.np)) {
		ARISC_ERR("get [allwinner,s_scpucfg] device node error\n");
		return -EINVAL;
	}

	ret = of_address_to_resource(arisc_cfg.scpucfg.np, 0, &res);
	if (ret || !res.start) {
		ARISC_ERR("get s_scpucfg pbase error\n");
		return -EINVAL;
	}
	arisc_cfg.scpucfg.pbase = res.start;
	arisc_cfg.scpucfg.size = resource_size(&res);

	arisc_cfg.scpucfg.vbase = of_iomap(arisc_cfg.scpucfg.np, 0);
	if (!arisc_cfg.scpucfg.vbase)
		panic("Can't map scpucfg registers");
	ARISC_INF("scpucfg pbase:0x%p, vbase:0x%p, size:0x%x\n",
		(void *)arisc_cfg.scpucfg.pbase, arisc_cfg.scpucfg.vbase,
		arisc_cfg.scpucfg.size);
...
int sunxi_deassert_arisc(void)
{
	...
	value = readl(arisc_cfg.scpucfg.vbase + 0x0);
	value &= ~1;
	writel(value, arisc_cfg.scpucfg.vbase + 0x0);
	value = readl(arisc_cfg.scpucfg.vbase + 0x0);
	value |= 1;
	writel(value, arisc_cfg.scpucfg.vbase + 0x0);
