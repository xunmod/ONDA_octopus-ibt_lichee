...
#define AW_SENSOR_ODR    (100)
#define AW_SENSOR_OFFSET (1000*1000*1000/AW_SENSOR_ODR)
...
static irqreturn_t inv_irq_handler(int irq, void *dev_id)
{
	...
	while ((time_since_last_irq > st->irq_dur_ns * 2) &&
	       ...
	       (!st->chip_config.lpa_mode) && (!st->chip_config.dmp_on)) {
		st->last_isr_time += st->irq_dur_ns;
		kfifo_in(&st->timestamps, &st->last_isr_time, 1);
		time_since_last_irq = timestamp - st->last_isr_time;
		catch_up++;
	}
...
irqreturn_t inv_read_fifo_mpu3050(int irq, void *dev_id)
{
	...
	if (byte_read != 0) {
		...
		if (fifo_count > FIFO_THRESHOLD)
			goto flush_fifo;
		/* Timestamp mismatch. */
		if (kfifo_len(&st->timestamps) < fifo_count / byte_read)
			goto flush_fifo;
		...
		if (compass_divider == st->compass_counter) {
			if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
				...
				result = inv_i2c_read(st, REG_EXT_SENS_DATA_00,
						      NUM_BYTES_COMPASS_SLAVE_AK09911,
						      d);
...
static int get_bytes_per_datum(struct inv_mpu_iio_s *st)
{
	...
	if (st->chip_config.dmp_on) {
		...
		if (st->chip_config.tap_on || st->chip_config.display_orient_on)
...
irqreturn_t inv_read_fifo(int irq, void *dev_id)
{
	...
	if (!(st->chip_config.accl_fifo_enable |
	      ...
	      st->chip_config.compass_fifo_enable | st->mot_int.mot_on))
		goto end_session;
	if (st->mot_int.mot_on)
...
static void inv_scan_query(struct iio_dev *indio_dev)
{
	...
	if (st->chip_config.gyro_fifo_enable && (!st->chip_config.gyro_enable)) {
		result = st->switch_gyro_engine(st, true);
		if (result)
			return;
		st->chip_config.gyro_enable = true;
	}
	if (st->chip_config.accl_fifo_enable && (!st->chip_config.accl_enable)) {
...
static int inv_check_quaternion(struct iio_dev *indio_dev)
{
	...
	if (st->chip_config.dmp_on) {
		...
		result = inv_send_quaternion(st, st->chip_config.quaternion_on);
		if (result)
			return result;
	} else {
		st->chip_config.quaternion_on = 0;
...
{
    struct inv_mpu_iio_s *st = container_of(work, struct inv_mpu_iio_s, work);
    struct iio_dev *indio_dev = iio_priv_to_dev(st);
    long long this_run_time = get_time_ns();
    ...
    swfifolen = kfifo_len(&st->timestamps);

    if (swfifolen > 100)
		pr_err("aw check. swfifolen = %d \n", swfifolen);
	...
	if (result) {
		pr_err("## aw read i2c failed \n");
		awerr = 5;
		goto end_session;
	}
	fifo_count = be16_to_cpup((__be16 *)(&data[0]));
	hwfifolen = fifo_count / bytes_per_datum;
	...
	if (swfifolen > fifo_count / bytes_per_datum + TIME_STAMP_TOR) {
		if (st->chip_config.dmp_on) {
		result = kfifo_to_user(&st->timestamps, &timestamp, sizeof(timestamp), &copied);
		if (result) {
			awerr = 10;
			goto flush_fifo;
			}
			...
			/*goto flush_fifo;*/
			pr_err("aw: swfifo=%d >> hwfifo=%d", swfifolen, fifo_count);
			}
		}
	} else  {
	result = kfifo_to_user(&st->timestamps, &timestamp, sizeof(timestamp), &copied);
	...
	/*only report partial data to user.
    //slow drifting!*/
	int read_datum_count = (swfifolen <= hwfifolen) ? swfifolen : hwfifolen;
	while (read_datum_count--) {
		result = inv_i2c_read(st, reg->fifo_r_w, bytes_per_datum, data);
		if (result) {
			awerr = -11;
			goto flush_fifo;
...
end_session:
	...
	mutex_unlock(&indio_dev->mlock);
	if (awerr != 0)
		pr_err("aw: end_session: %d \n", awerr);
	return;
flush_fifo:
	/* Flush HW and SW FIFOs. */
	pr_err("aw: work_count between flush: %ld \n", work_count);
	work_count = 0;
	pr_err("##fuck: awerr = %d. time=%lld. fifolen{%d,%d}, bytes_per_datum=%d \n", awerr, this_run_time, hwfifolen, swfifolen, bytes_per_datum);
	inv_reset_fifo(indio_dev);
	mutex_unlock(&st->mutex);
	mutex_unlock(&indio_dev->mlock);
	return;
...
static int invsens_request_irq(struct inv_mpu_iio_s *st)
{
	...
	ret = request_threaded_irq(irq_number, inv_irq_handler,
			inv_read_fifo,
			IRQF_TRIGGER_FALLING, "inv_irq", st);
		if (IS_ERR_VALUE(ret)) {
...
int inv_mpu_configure_ring(struct iio_dev *indio_dev)
{
	...
	sensor_info.dev = &(indio_dev->dev);
	ret = invsens_request_irq(st);
