/*
 *  auto_detect.c - Linux kernel modules for  Detection i2c device.
 */
...
#define dprintk(level_mask, fmt, arg ...) \
		do { if (unlikely(debug_mask & level_mask)) {printk(KERN_DEBUG fmt, ## arg); } } while (0)

module_param_named(debug_mask, debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
module_param_named(ctp_mask, ctp_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);

/*gsensor info*/
static struct auto_detect_info gsensors[] = {
	{"lsm9ds0_acc_mag", 0, {0x1e, 0x1d}, 0x0f, {0x49}, 0},
	{"bma250", 	     0, {0x18, 0x19, 0x08, 0x38}, 0x00, {0x02, 0x03, 0xf9, 0xf8, 0xfa}, 0},
...
/*ctp info*/
static struct auto_detect_info ctps[] = {
...
static struct auto_detect_info lsensors[] = {
...
static struct auto_detect_getted_flag	device_getted_flag;
...
static struct auto_detect_name d_name = {"", "", "", "", "", 0, 0, 0, 0, 0};
static void auto_detects_events(struct work_struct *work);


static struct workqueue_struct *sw_wq;
static DECLARE_WORK(sw_work, auto_detects_events);
...
static int get_power_para(struct para_power *pm)
{
	...
	dprintk(DEBUG_INIT, "[auto_detect]:%s: power_ldo = %s, power_ldo_vol = %d, "
...
static int get_device_same_addr(struct auto_detect *sw, int now_number)
{
	int	ret		= -1;
	int	i		= 0;
...
static int get_device_name_number(struct auto_detect *sw)
{
	int	ret	= -1;
	int	number	= sw->support_number;
...
static void get_detect_list(struct auto_detect *sw)
{
	int	i	= 0;
	int	val	= -1;
	int	number	= sw->support_number;
...
static int sw_sysconfig_get_para(struct auto_detect *sw)
{
	int	ret		= -1;
	int	device_used	= 0;
...
static bool sw_match_chip_id(struct auto_detect_info *info, int number, int value)
{
	int i = 0;
...
static int sw_chip_id_gain(struct auto_detect *sw, int now_number)
{
...
static int sw_chip_id_detect(struct auto_detect *sw, int now_number)
{
	int	result		= -1;
	int	same_number	= 0;
...
static int auto_detect_response_test(struct auto_detect *sw, int now_number)
{
...
static int sw_i2c_detect(struct auto_detect *sw)
{
	int	number		= sw->support_number;
	int	now_number	= -1;
	int	scan_number	= 0;
	...
	while (scan_number < number) {
		...
		ret = auto_detect_response_test(sw, now_number);
		if (!ret) {
			now_number++;
			continue;
		}
...
static int auto_detect_detect_start(struct auto_detect *sw)
{
...
static int sw_register_device_detect(struct auto_detect_info info[], struct para_name *name,
				      int number)
{
	struct auto_detect	*sw;
	struct i2c_client	*client;

	dprintk(DEBUG_INIT, "[auto_detect]:%s begin!\n", __func__);
	...
	if (auto_detect_detect_start(sw) < 0) {
	...
	dprintk(DEBUG_INIT, "[auto_detect]:%s successed !\n", __func__);

	return 0;
}


static void auto_detects_set_power(struct para_power *pm)
{
	if (pm->power_ldo) {
...
static void auto_detects_set_reset_gpio(struct para_power *wakeup_gpio)
{
	if (gpio_is_valid(wakeup_gpio->reset_pin.gpio)) {
		if (0 != gpio_request(wakeup_gpio->reset_pin.gpio, NULL))
			printk(KERN_ERR "wakeup gpio_request is failed\n");
		else
			gpio_direction_output(wakeup_gpio->reset_pin.gpio, wakeup_gpio->reset_pin.data);
	}
}


static void auto_detects_reset_gpio_free(struct para_power *wakeup_gpio)
{
	if (gpio_is_valid(wakeup_gpio->reset_pin.gpio))
		gpio_free(wakeup_gpio->reset_pin.gpio);
}

static void auto_detects_power_free(struct para_power *pm)
{
	if (pm && pm->ldo && !IS_ERR(pm->ldo)) {
...
static void auto_detects_events(struct work_struct *work)
{
	...
	auto_detects_set_power(&c_power);
	auto_detects_set_reset_gpio(&c_power);
	dprintk(DEBUG_INIT, "[auto_detect]:%s begin!\n", __func__);
	pr_err("[auto_detect]:%s begin! ctp_mask :%d\n", __func__, ctp_mask);
	if (ctp_mask != 1) {
	...
	auto_detects_power_free(&c_power);
	auto_detects_reset_gpio_free(&c_power);
	detect_end_flag = 1;
	dprintk(DEBUG_INIT, "[auto_detect]:%s end! Totally %d devices uesed\n", __func__, dev_count);
}


static ssize_t auto_detect_gsensor_show(struct device *dev,
					 struct device_attribute *attr, char *buf)
{
	ssize_t cnt = 0;
	dprintk(DEBUG_INIT, "[auto_detect]:%s: device name:%s\n", __func__, d_name.g_name);
...
static ssize_t auto_detect_gsensor_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t auto_detect_ctp_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	ssize_t cnt = 0;
...
static ssize_t auto_detect_ctp_store(struct device *dev, struct device_attribute *attr,
				      const char *buf, size_t size)
{
	return 0;
}
...
static ssize_t auto_detect_devlist_store(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	return 0;
}
...
static DEVICE_ATTR(gsensor, S_IRUGO | S_IWUSR | S_IWGRP, auto_detect_gsensor_show, auto_detect_gsensor_store);
static DEVICE_ATTR(ctp, S_IRUGO | S_IWUSR | S_IWGRP, auto_detect_ctp_show, auto_detect_ctp_store);
static DEVICE_ATTR(devlist, S_IRUGO | S_IWUSR | S_IWGRP, auto_detect_devlist_show, auto_detect_devlist_store);
...
struct device_node *find_np_by_name(const char *name)
{
	...
	dprintk(DEBUG_INIT, "[auto_detect] cat't find node in all_device_np, name : %s\n", name);
	return NULL;
}
...
static int sw_input_probe(struct platform_device *pdev)
{
	...
	static int		count;
	if (!np) {
		printk("[auto_detect]: ERROR get device tree node failed\n");
		return -1;
	}

	dprintk(DEBUG_INIT, "[auto_detect] probe name:%s, node:%p\n", np->name, np);
...
static const struct of_device_id sw_input_match[] = {
	{.compatible = "allwinner,sun8i-ctp-para", 		},
	{.compatible = "allwinner,sun8i-ctp-list", 		},
	{.compatible = "allwinner,sun8i-gsensor-para", 		},
	{.compatible = "allwinner,sun8i-gsensor-list-para", 	},
	{.compatible = "allwinner,sun8i-compass-para", 		},
	{.compatible = "allwinner,sun8i-compass-list-para", 	},
	{.compatible = "allwinner,sun8i-lsensors-para", 	},
	{.compatible = "allwinner,sun8i-lsensors-list-para", 	},
	{.compatible = "allwinner,sun8i-gyr_sensors-para", 	},
	{.compatible = "allwinner,sun8i-gyr_sensors-list-para", },
	{},
};
#else
...
static struct attribute *auto_detect_attributes[] = {
...
static void auto_detect_release(struct device *dev)
{
}


static struct device auto_detect_detect = {
	.init_name	= "auto_detect",
	.release	= auto_detect_release,
};

static int __init auto_detect_init(void)
{
	int err = 0;
	dprintk(DEBUG_INIT, "[auto_detect]:%s begin!\n", __func__);
	...
	dprintk(DEBUG_INIT, "[auto_detect]: register platform driver: sw_input_platform_driver\n");
	platform_driver_register(&sw_input_platform_driver);


	queue_work(sw_wq, &sw_work);

	auto_detect_detect.groups	= dev_attr_groups;
	err				= device_register(&auto_detect_detect);
	if (err) {
		printk("%s register camera detect driver as misc device error\n", __FUNCTION__);
	}
	dprintk(DEBUG_INIT, "[auto_detect]:%s end!\n", __func__);
	return 0;
}


static void __exit auto_detect_exit(void)
{
	printk(" sw device driver exit!\n");
	device_unregister(&auto_detect_detect);
...
module_init(auto_detect_init);
module_exit(auto_detect_exit);
