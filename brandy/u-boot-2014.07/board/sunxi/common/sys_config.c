...
volatile void* PIO_REG_CFG(int port, int port_num)
{
	if(port < 12)
	{
		return _PIO_REG_CFG(port, (port_num>>3));   //更新功能寄存器地址

	}
	else
	{
		return _R_PIO_REG_CFG(port, (port_num>>3));  
	}
}
...
volatile void* PIO_REG_PULL(int port, int port_num)
{
	if(port < 12)
	{
		return _PIO_REG_PULL(port, (port_num>>4));  
	}
	else
	{
		return _R_PIO_REG_PULL(port, (port_num>>4));  
	}
}
...
volatile void* PIO_REG_DLEVEL(int port,int port_num)
{
	if(port < 12)
	{
		return _PIO_REG_DLEVEL(port, (port_num>>4));
	}
	else
	{
		return _R_PIO_REG_DLEVEL(port, (port_num>>4));
	}
}
...
************************************************************************************************************
*/
ulong gpio_request_ex(char *main_name, const char *sub_name)  //设备申请GPIO函数扩展接口
{
...
__s32 gpio_release(ulong p_handler, __s32 if_release_to_default_status)
{
    char               *tmp_buf;                                        //转换成char类型
    __u32               group_count_max, first_port;                    //最大GPIO个数
    system_gpio_set_t  *user_gpio_set, *tmp_sys_gpio_data;
