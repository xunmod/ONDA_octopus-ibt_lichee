 ...
 * Description: MMC driver for  mmc controller operations of sun8iw6
 * Author:
 * Date: 2016/10/14 19:15:00
 */
#include <common.h>
#include <asm/io.h>
...
static int mmc_set_mclk(struct sunxi_mmc_host* mmchost, u32 clk_hz)
{
	...
	if (clk_hz <= 4000000) { //400000
		...
		src = 1;
		sclk_hz = sunxi_clock_get_pll6() * 1000000; /*use 2x pll-per0 clock */
	}
...
static unsigned mmc_get_mclk(struct sunxi_mmc_host* mmchost)
{
	...
	if (src == 0)
		...
		sclk_hz = sunxi_clock_get_pll6() * 1000000; /* use 2x pll6 */
		MMCDBG("%s: pll %d Hz\n", __func__, sclk_hz);
	} else {
...
static unsigned mmc_config_delay(struct sunxi_mmc_host* mmchost)
{
	...
	{
		spd_md = mmchost->tm0.cur_spd_md;
		freq = mmchost->tm0.cur_freq;
		...
		if (mmchost->tm0.odly[spd_md*MAX_CLK_FREQ_NUM+freq] != 0xFF) {
			odly = mmchost->tm0.odly[spd_md*MAX_CLK_FREQ_NUM+freq];
			//MMCDBG("%s: odly: %d   cfg \n", __FUNCTION__, odly);
		} else {
			odly = mmchost->tm0.def_odly[spd_md*MAX_CLK_FREQ_NUM+freq];
			//MMCDBG("%s: odly: %d   default \n", __FUNCTION__, odly);
		}
		if (mmchost->tm0.sdly[spd_md*MAX_CLK_FREQ_NUM+freq] != 0xFF) {
			sdly = mmchost->tm0.sdly[spd_md*MAX_CLK_FREQ_NUM+freq];
			//MMCDBG("%s: sdly: %d   cfg \n", __FUNCTION__, sdly);
		} else {
			sdly = mmchost->tm0.def_sdly[spd_md*MAX_CLK_FREQ_NUM+freq];
			//MMCDBG("%s: sdly: %d   default \n", __FUNCTION__, sdly);
		}
		MMCDBG("%s: odly: %d   sldy: %d\n", __FUNCTION__, odly, sdly);
		...
		if (mmc_update_clk(mmchost)) {
			MMCINFO("%s: mmc_update_clk fail!! \n", __FUNCTION__);
			return -1;
		}
	}
	else
	{
		MMCINFO("%s: wrong timing mode %d\n", __FUNCTION__, mode);
		return -1;
	}
...
static int mmc_config_clock_modex(struct sunxi_mmc_host* mmchost, unsigned clk)
{
	...
	if ((mode == SUNXI_MMC_TIMING_MODE_1)) {
		...
		if ((mmc->speed_mode == HSDDR52_DDR50)
			&& (mmc->bus_width == 8))
			mmchost->mod_clk = clk * 2; /* xxclk: DDR8(HS) */
		else
			mmchost->mod_clk = clk ; /* 1xclk: SDR 1/4/8; DDR4(HS); DDR8(HS400)  */
	}
	...
	if (mode == SUNXI_MMC_TIMING_MODE_1) {
		...
		mmchost->tm0.cur_spd_md = mmchost->mmc->speed_mode;
		mmchost->tm0.cur_freq = freq_id;
	}

	mmc_config_delay(mmchost);
	MMCDBG("mmc %d mclkbase 0x%x, ntsr 0x%x 0x%x\n", mmchost->mmc_no,\
		readl(mmchost->mclkbase), readl(&reg->ntsr), readl(&reg->ntdbg));

#else

	MMCINFO("%s: fatal error!!!!!\n", __FUNCTION__);
...
static int mmc_config_clock(struct sunxi_mmc_host* mmchost, unsigned clk)
{
	...
	if ((mmchost->timing_mode == SUNXI_MMC_TIMING_MODE_1)
		|| (mmchost->timing_mode == SUNXI_MMC_TIMING_MODE_0) )
...
#if 0 // don't support HS400 mode for sun8iw6
...
#else
	writel((2U<<28)|(7<<16)|8, &reg->ftrglevel);
#endif
...
static int sunxi_decide_rty(struct mmc *mmc, int err_no, uint rst_cnt)
{
	...
	if (err_no && (!(err_no & SDXC_RespTimeout)||(err_no==0xffffffff)))
	{
		...
		else if (tmode == SUNXI_MMC_TIMING_MODE_0)
		{
			spd_md = mmchost->tm0.cur_spd_md;
			freq = mmchost->tm0.cur_freq;
			sdly = &mmchost->tm0.sdly[spd_md*MAX_CLK_FREQ_NUM+freq];

			if (mmchost->retry_cnt * tm0_retry_gap <  MMC_CLK_SAMPLE_POINIT_MODE_0) {
				if ( (*sdly + tm0_retry_gap) < MMC_CLK_SAMPLE_POINIT_MODE_0) {
					*sdly = *sdly + tm0_retry_gap;
				} else {
					*sdly = *sdly + tm0_retry_gap - MMC_CLK_SAMPLE_POINIT_MODE_0;
				}
				MMCINFO("Get next samply point %d at spd_md %d freq_id %d\n", *sdly, spd_md, freq);
			} else {
				MMCINFO("Beyond the retry times\n");
