...
#define TEESMC_OPTEE_SIP_SUNXI_READ_REG			    (0xFFFF0000)
#define TEESMC_OPTEE_SIP_SUNXI_WRITE_REG		    (0xFFFF0001)
#define TEESMC_OPTEE_SIP_SUNXI_CPU_POWERUP		    (0xFFFF0002)
#define TEESMC_OPTEE_SIP_SUNXI_CPU_POWERDOWN		(0xFFFF0003)
#define TEESMC_OPTEE_SIP_SUNXI_CLUSTER_POWERUP		(0xFFFF0004)
#define TEESMC_OPTEE_SIP_SUNXI_CLUSTER_POWERDOWN	(0xFFFF0005)
#define TEESMC_OPTEE_SIP_SUNXI_LOAD_ARISC		    (0xFFFF0006)
#define TEESMC_OPTEE_SIP_SUNXI_SET_SMP_BOOTENTRY	(0xFFFF0007)
#define TEESMC_OPTEE_SIP_SUNXI_GET_DRM_MEM_INFO 	(0xFFFF0008)
#define TEESMC_OPTEE_SIP_SUNXI_GET_DRM_MEM_INFO 	(0xFFFF0008)

#define TEESMC_OPTEE_SIP_SUNXI_EFUSE_WRITE          (0xFFFF000A)
#define TEESMC_OPTEE_SIP_SUNXI_EFUSE_READ           (0xFFFF000B)

#define TEE_SMC_INIT_CALL		                    (0x0FFFFFF1)
#define TEE_SMC_PLAFORM_OPERATION                   (0x0FFFFFF2)
#define TEE_SMC_PLAFORM_RWREG                       (0x0FFFFFFC)
...
{
	return sunxi_smc_call(TEE_SMC_PLAFORM_RWREG, TEESMC_OPTEE_SIP_SUNXI_READ_REG, reg, 0, 0);
}

int arm_svc_write_sec_reg(u32 val,ulong reg)
{
	sunxi_smc_call(TEE_SMC_PLAFORM_RWREG, TEESMC_OPTEE_SIP_SUNXI_WRITE_REG, reg, val, 0);
	return 0;
...
{
	//asm volatile ("b .");
	return sunxi_smc_call(TEE_SMC_PLAFORM_OPERATION, TEESMC_OPTEE_SIP_SUNXI_EFUSE_READ, (ulong)key_buf, (ulong)read_buf, 0);
}

int arm_svc_efuse_write(void *key_buf)
{
	return sunxi_smc_call(TEE_SMC_PLAFORM_OPERATION, TEESMC_OPTEE_SIP_SUNXI_EFUSE_WRITE,(ulong)key_buf, 0, 0);
}
...
int smc_efuse_readl(void *key_buf, void *read_buf)
{
	...
	shm_base = (char *)param.a1;
	memset(shm_base, 0, 1024);
	memset(&param, 0, sizeof(param));
	param.a0 = TEE_SMC_PLAFORM_RWREG;
	param.a1 = TEESMC_OPTEE_SIP_SUNXI_EFUSE_READ;
	param.a2 = (uint32_t)shm_base;
	param.a3 = (uint32_t)(shm_base + 64);

	strcpy((void *)param.a2, key_buf);
	strcpy((void *)param.a3, read_buf);
	flush_cache((unsigned long)shm_base, 1024);

	tee_smc_call(&param);

	if (param.a0 <= 0) {
		printf("smc tee efuse read failed\n ");

		return -1;
	}
	memcpy(read_buf, (void *)(shm_base + 64), param.a0);
...
int smc_efuse_writel(void *key_buf)
{
	struct smc_param param = { 0 };
	char *shm_base = NULL;
	sunxi_efuse_key_info_t *efuse_key;
	...
	shm_base = (char *)param.a1;
	efuse_key = (sunxi_efuse_key_info_t *)shm_base;
	memcpy(shm_base, key_buf, 128);
	//sunxi_dump(efuse_key->data_addr, 32);
	memcpy((u8 *)shm_base + offsetof(sunxi_efuse_key_info_t, key_data), efuse_key->data_addr, efuse_key->len);
	flush_cache((unsigned long)shm_base, 1024);

	memset(&param, 0, sizeof(param));
	param.a0 = TEE_SMC_PLAFORM_RWREG;
	param.a1 = TEESMC_OPTEE_SIP_SUNXI_EFUSE_WRITE;
	param.a2 = (uint32_t)shm_base;
	tee_smc_call(&param);

	if (param.a0 <= 0) {
		printf("smc tee efuse write failed\n ");
...
int tee_main_entry(void)
{
	int ret;

	printf("semelis initialize\n");
	ret = sunxi_smc_call(TEE_SMC_INIT_CALL, 0, 0, 0, 0);
	if (ret == 0) {
